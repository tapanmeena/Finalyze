...
export default function BillsScreen() {
  ...
  const getDueStatus = (dueDate: string, isPaid: boolean) => {
    if (isPaid) return { text: 'Paid', color: theme.colors.success };
    
    const today = new Date();
    const currentDay = today.getDate();
    const dueDateNum = parseInt(dueDate);
    
    if (currentDay === dueDateNum) return { text: 'Due Today', color: theme.colors.error };
    if (currentDay > dueDateNum) return { text: 'Overdue', color: theme.colors.error };
    
    const daysUntilDue = dueDateNum - currentDay;
    if (daysUntilDue <= 3) return { text: `Due in ${daysUntilDue} days`, color: theme.colors.warning };

    return { text: `Due on ${dueDate}`, color: theme.colors.textSecondary };
  };
